...
class OrdArray {
    ...
    public static OrdArray merge1(OrdArray a , OrdArray b , int size){
        int s = a.size() + b.size();
        OrdArray c = new OrdArray(s);

        for(int i=0; i<a.size() ; i++){
            c.insert(a.getElementFromPos(i));
        }

        for(int j=0; j<b.size() ; j++){
            c.insert(b.getElementFromPos(j));
        }
...
class orderedArray {
	...
	public static void main(String[] args) {
        ...
        OrdArray c = OrdArray.merge1(arr, arr1, maxSize);
        c.display();
